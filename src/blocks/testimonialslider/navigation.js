...
class TestimonialSliderNavigation extends Component {
	render() {
		const {
			addSlide,
			activateSlide,
		...
		const slides = slidesOrder.map((slide, index) => {
			return {
				...
				label: sprintf(
					__('Testimonial Slide %d', 'gbblocks'),
					index + 1
				),
			};
		});
		...
		return (
			...
			>
				<CardBody className="wp-block-gbblocks-content-slider__controls-wrapper">
					<div className="wp-block-gbblocks-content-slider__controls-actions">
						<Button
							label={__('Edit Testimonial Slide', 'gbblocks')}
							variant="secondary"
							icon="edit"
							disabled={!isEditActive}
							onClick={() => {
						...
						>
							{__('Edit Testimonial Slide', 'gbblocks')}
						</Button>
					</div>
...
export default TestimonialSliderNavigation;
