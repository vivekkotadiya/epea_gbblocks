...
function ReadOnlyContent({ userCanEdit, postType, postId }) {
	...
	return content?.protected && !userCanEdit ? (
		<div {...blockProps}>
			<Warning>
				{__('This content is password protected.', 'gbblocks')}
			</Warning>
		</div>
	) : (
		<div
...
function EditableContent({ layout, context = {} }) {
	...
	const props = useInnerBlocksProps(useBlockProps({ className: '' }), {
		value: blocks,
		onInput,
		onChange,
...
function RecursionError() {
	...
	return (
		<div {...blockProps}>
			<Warning>
				{__('Block cannot be rendered inside itself.', 'gbblocks')}
			</Warning>
		</div>
	);
}
